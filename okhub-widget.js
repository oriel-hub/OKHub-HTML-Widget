		...
		jQuery
				.getJSON(
						...
						function(data) {
							if (typeof data.metadata.next_page != "undefined") {
								footer = footer
										+ "<button id='okhub_next' style='float:right;'> Next >> </button>";
							}
							if (typeof data.metadata.prev_page != "undefined") {
								footer = footer
										+ "<button id='okhub_prev' style='float:left;'> << Previous</button>";
							}
							output = output + "<center><h5>"
									+ data.metadata.total_results
								...
								jQuery
										.each(
												...
												function(index, value) {
													var title = typeof value.en != "undefined" ? value.en[0]
															: value.fr[0];
													output = output
															+ "<p id='okhub_"
															+ data.results[i].item_id
															+ "' class='okhub_item' style='cursor:hand;'>"
															+ title + "</p>";
													if (title != "") {
														return false;
													}
							...
							jQuery('#open-knowledge-hub-widget-content').html(
									output);
							for ( var i = 0; i < data.results.length; i++) {
								jQuery("#okhub_" + data.results[i].item_id)
										.click(
												function(e) {
